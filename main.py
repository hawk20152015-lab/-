...
def _register_arabic_font():
    ...
    # يبحث عن الخطوط في مجلد التطبيق مباشرة (مسطّح) أو في assets/fonts
    search_dirs = [base, os.path.join(base, "assets", "fonts"),
                   os.path.join(base, "assets")]
    ttfs = []
    for d in search_dirs:
        if os.path.isdir(d):
            ttfs += [os.path.join(d, f) for f in os.listdir(d)
...
# ----------------------------------------------------------------------
# نظام ألوان عصري (إنديغو + تيل) — مستوحى من تطبيقات المال الحديثة
# ----------------------------------------------------------------------
C_PRIMARY = (0.169, 0.176, 0.420, 1)    # #2B2D6B إنديغو عميق
C_GRAD_1 = (0.231, 0.227, 0.490, 1)     # #3B3A7D بداية التدرّج
C_GRAD_2 = (0.078, 0.682, 0.620, 1)     # #14AE9E نهاية التدرّج (تيل)
C_ACCENT = (0.078, 0.722, 0.651, 1)     # #14B8A6 تيل
C_BG = (0.957, 0.961, 0.984, 1)         # #F4F5FB خلفية فاتحة ناعمة
C_SURFACE = (1, 1, 1, 1)                 # بطاقات بيضاء
C_SELECTED = (0.918, 0.914, 0.984, 1)   # #EAE9FB تظليل التحديد
C_POSITIVE = (0.063, 0.725, 0.506, 1)   # #10B981 أخضر زمردي (إضافة)
C_NEGATIVE = (0.957, 0.247, 0.369, 1)   # #F43F5E وردي (نثرية)
C_DISBURSE = (0.961, 0.620, 0.043, 1)   # #F59E0B كهرماني (صرف)
C_TEXT = (0.122, 0.137, 0.208, 1)       # #1F2335
C_MUTED = (0.42, 0.45, 0.52, 1)         # نص ثانوي

SPECIAL_DIVISION = "المنقولون إلى الخارج"


# ----------------------------------------------------------------------
# تدرّج لوني عصري (Hero card)
# ----------------------------------------------------------------------
GRAD_TEX = None


def _make_gradient(c1, c2, size=128):
    """ينشئ نسيج تدرّج أفقي بين لونين (يُستدعى بعد تهيئة GL)."""
    tex = Texture.create(size=(size, 1), colorfmt="rgba")
    buf = bytearray()
    for i in range(size):
        t = i / (size - 1)
        for ch in range(4):
            buf.append(max(0, min(255,
                       int((c1[ch] * (1 - t) + c2[ch] * t) * 255))))
    tex.blit_buffer(bytes(buf), colorfmt="rgba", bufferfmt="ubyte")
    tex.wrap = "clamp_to_edge"
    return tex


class GradientCard(MDBoxLayout):
    """صندوق بخلفية تدرّج لوني وزوايا دائرية (Hero)."""
    radius_all = NumericProperty(20)
    ...
    def __init__(self, **kw):
        ...
        with self.canvas.before:
            ...
            self._rect = RoundedRectangle(pos=self.pos, size=self.size,
                                          radius=[self.radius_all] * 4)
        self._apply()
        self.bind(pos=self._upd, size=self._upd, radius_all=self._upd)

    def _apply(self):
        global GRAD_TEX
        if GRAD_TEX is None:
            try:
                GRAD_TEX = _make_gradient(C_GRAD_1, C_GRAD_2)
            except Exception:
                GRAD_TEX = None
        if GRAD_TEX is not None:
            self._rect.texture = GRAD_TEX

    def _upd(self, *a):
        self._rect.pos = self.pos
        self._rect.size = self.size
        self._rect.radius = [self.radius_all] * 4


Factory.register("GradientCard", cls=GradientCard)
...
class DB:
    ...
    def _create(self):
        ...
        self._ensure_column(c, "transactions", "kind", "TEXT DEFAULT ''")
        # شعبة خاصة دائمة للمنقولين إلى الخارج
        c.execute("INSERT OR IGNORE INTO divisions(name) VALUES(?)",
                  (SPECIAL_DIVISION,))
        self.conn.commit()
    ...
    def division_nathriya(self, did):
        ...
        r = self.conn.execute(
            """SELECT COALESCE(SUM(-t.amount),0) AS p
               ...
               WHERE m.division_id=? AND t.amount<0 AND t.kind!='disburse'""", (did,)).fetchone()
    ...
    def get_divisions(self):
        q = """SELECT d.id, d.name, d.base_amount, d.donation,
                      ...
                      COALESCE((SELECT SUM(-t.amount) FROM transactions t
                                ...
                                WHERE mm.division_id=d.id AND t.amount<0 AND t.kind!='disburse'),0)
                          AS nathriya
               FROM divisions d
    ...
    # ----- المعاملات -----
    def add_transaction(self, member_id, amount, note="", kind=""):
        now = datetime.now().strftime("%Y-%m-%d %H:%M")
        self.conn.execute(
            "INSERT INTO transactions(member_id, amount, note, created_at, kind) "
            "VALUES(?,?,?,?,?)", (member_id, amount, note, now, kind))
        self.conn.execute(
    ...
    def nathriya_collected(self):
        ...
        r = self.conn.execute(
            ...
            "WHERE amount<0 AND kind!='disburse'").fetchone()
        return r["p"] if r else 0
...
class TxRow(MDBoxLayout):
    ...
    amount_color = ColorProperty(C_POSITIVE)
...
class DivisionsScreen(Screen):
    ...
    def refresh(self):
        ...
        app.nathriya_str = f"{app.db.nathriya_available():.0f}"
        ...
        for r in app.db.get_divisions():
            ...
            card = DivisionCard()
            card.div_id = r["id"]
            card.special = (r["name"] == SPECIAL_DIVISION)
...
class MembersScreen(Screen):
    ...
    def move_member_menu(self, member_id, caller):
        app = MDApp.get_running_app()
        items = []
        for d in app.db.get_divisions():
            if d["id"] == self.division_id:
                continue
            items.append({
                "viewclass": "OneLineListItem",
                "text": ar(d["name"]),
                "height": dp(48),
                "on_release": (lambda did=d["id"]: self._do_move(member_id, did)),
            })
        if not items:
            toast("لا توجد شعبة أخرى")
            return
        self._move_menu = MDDropdownMenu(caller=caller, items=items, width_mult=4)
        self._move_menu.open()

    def _do_move(self, member_id, new_div):
        if getattr(self, "_move_menu", None):
            self._move_menu.dismiss()
        MDApp.get_running_app().db.move_member(member_id, new_div)
        toast("تم نقل الفرد")
        self.refresh()
...
class MemberScreen(Screen):
    ...
    def refresh(self):
        ...
        for t in app.db.get_transactions(self.member_id):
            ...
            row = TxRow()
            if amt >= 0:
                row.amount_color = C_POSITIVE
                sign = "+"
            elif kind == "disburse":
                row.amount_color = C_DISBURSE
                sign = "-"
            else:
                row.amount_color = C_NEGATIVE
                sign = "-"
            row.amount_ar = f"{sign}{abs(amt):.0f}"
    ...
    def _tx(self, title, positive, kind):
        app = MDApp.get_running_app()
        app.amount_dialog(
            title=title, show_sign=False,
            on_ok=lambda amount, note: (
                app.db.add_transaction(
                    self.member_id, amount if positive else -amount, note, kind),
                self.refresh()))

    def add_money(self):
        self._tx("إضافة مبلغ", True, "add")

    def deduct_nathriya(self):
        self._tx("خصم للنثرية", False, "nathriya")

    def disburse(self):
        self._tx("صرف من الحساب", False, "disburse")
...
<DivisionCard>:
    ...
    radius: [dp(20)]
    elevation: 0
    line_color: (app.c_accent if root.special else (0.90,0.91,0.96,1))
    line_width: 1.2
    ...
    MDBoxLayout:
        ...
        spacing: dp(2)
        MDIcon:
            icon: "airplane-takeoff" if root.special else "folder-account"
            size_hint_x: None
            width: dp(26)
            theme_text_color: "Custom"
            text_color: app.c_accent if root.special else app.c_primary
        MDLabel:
            text: root.name_ar
            halign: "right"
            font_style: "H6"
            bold: True
            theme_text_color: "Custom"
            text_color: 0.122,0.137,0.208,1
        MDIconButton:
            icon: "cash-multiple"
            theme_text_color: "Custom"
            text_color: app.c_accent
            on_release: app.root.get_screen("divisions").edit_amounts(root.div_id)
        MDIconButton:
            icon: "pencil"
            theme_text_color: "Custom"
            text_color: 0.55,0.57,0.63,1
            on_release: app.root.get_screen("divisions").edit_division(root.div_id, root.name_ar)
        MDIconButton:
            icon: "delete-outline"
            theme_text_color: "Custom"
            text_color: app.c_negative
            on_release: app.root.get_screen("divisions").delete_division(root.div_id)
    MDLabel:
        text: root.count_ar
        halign: "right"
        font_style: "Caption"
...
<MemberCard>:
    ...
    radius: [dp(18)]
    elevation: 0
    line_color: (app.c_accent if root.selected else (0.90,0.91,0.96,1))
    line_width: 1.2
    ...
    MDCheckbox:
        size_hint_x: None
        width: dp(34) if root.selecting else 0
    ...
    MDIconButton:
        ...
        text_color: app.c_accent
        size_hint_x: None
        width: 0 if root.selecting else dp(40)
        opacity: 0 if root.selecting else 1
        disabled: root.selecting
        on_release:
            app.root.get_screen("members").move_member_menu(root.member_id, self)
    MDBoxLayout:
        orientation: "vertical"
        spacing: dp(1)
        MDLabel:
    ...
    MDCard:
        ...
        MDBoxLayout:
            ...
            MDLabel:
                ...
                font_style: "Body2"
                theme_text_color: "Custom"
                text_color: 0.20,0.22,0.30,1
            MDLabel:
                text: root.date_str
                halign: "right"
                font_style: "Caption"
                theme_text_color: "Custom"
                text_color: 0.6,0.62,0.68,1
...
<DivisionsScreen>:
    MDBoxLayout:
        ...
        MDTopAppBar:
            ...
            right_action_items: [["microsoft-excel", lambda x: root.import_excel()], ["plus", lambda x: root.open_add_dialog()]]
        GradientCard:
            ...
            MDRaisedButton:
                ...
                elevation: 0
                on_release: root.withdraw_nathriya()
            Widget:
            MDBoxLayout:
                orientation: "vertical"
                MDLabel:
...
<MembersScreen>:
    MDBoxLayout:
        ...
        MDBoxLayout:
            id: sel_bar
            size_hint_y: None
            height: dp(60) if root.selecting else 0
            opacity: 1 if root.selecting else 0
            disabled: not root.selecting
            padding: dp(12), dp(8)
            spacing: dp(10)
    ...
    MDBoxLayout:
        ...
        MDBoxLayout:
            ...
            MDRaisedButton:
                ...
                elevation: 0
                on_release: root.deduct_nathriya()
            MDRaisedButton:
                text: app.t_disburse
                md_bg_color: app.c_disburse
                elevation: 0
                on_release: root.disburse()
        ScrollView:
            MDBoxLayout:
                id: tx_list
                orientation: "vertical"
...
class HesabatApp(MDApp):
    ...
    t_apply_all = StringProperty(ar("للكل"))
    t_apply_sel = StringProperty(ar("للمحدد"))
    t_select_all = StringProperty(ar("تحديد الكل"))
    t_add_amount = StringProperty(ar("إضافة"))
    t_deduct_amount = StringProperty(ar("نثرية"))
    t_disburse = StringProperty(ar("صرف"))
    t_nathriya = StringProperty(ar("النثرية المتاحة"))
    t_withdraw = StringProperty(ar("سحب"))
    ...
    c_disburse = ColorProperty(C_DISBURSE)
    nathriya_str = StringProperty("0")
    ...
    def build(self):
        ...
        for _icon in (os.path.join(_base, "icon.png"),
